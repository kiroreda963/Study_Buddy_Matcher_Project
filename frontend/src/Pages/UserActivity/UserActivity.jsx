...
import { sessionClient, profileClient, authClient } from '../../clients/apolloClients';
import {
  GET_ME,
...
const UserProfile = ({ onBack }) => {
  ...
  // Queries
  const { data: userData, loading: userLoading, error: userError } = useQuery(GET_ME, { client: authClient });
  const { data: prefData, loading: prefLoading, error: prefError } = useQuery(GET_PROFILE_PREFERENCES, { client: profileClient });
  const { data: sessionData, loading: sessionLoading, error: sessionError } = useQuery(GET_STUDY_SESSIONS, { client: sessionClient });
  ...
  if (prefError) return <div className="user-profile-container">Error loading preferences: {prefError.message}</div>;
